...
**`open()` streams; `read_bytes`/`write_bytes` don't.** `open("rb")` and `open("wb")` stream the object in fixed-size chunks — writes use S3 multipart upload, so memory stays bounded regardless of object size and the 5 GB single-`PUT` limit doesn't apply. Use them for large objects. `read_bytes`/`write_bytes`/`read_text`/`write_text` load the whole object into memory, matching `pathlib` semantics. See [Getting Started → Large files](../getting-started.md#large-files-streaming).
